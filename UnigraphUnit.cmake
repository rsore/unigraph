...
function(_unigraph_make_unit_targets)
    ...
    foreach (unit IN LISTS unit_list)
        ...
        _unigraph_get_value_from_dict(LIST unit "windows_sources" target_windows_sources)
        _unigraph_get_value_from_dict(LIST unit "linux_sources" target_linux_sources)
        _unigraph_get_value_from_dict(LIST unit "darwin_sources" target_darwin_sources)
	...
	if(CMAKE_SYSTEM_NAME STREQUAL "Windows")
	  list(APPEND target_sources ${target_windows_sources})
	elseif(CMAKE_SYSTEM_NAME STREQUAL "Linux")
	  list(APPEND target_sources ${target_linux_sources})
	elseif(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
	  list(APPEND target_sources ${target_darwin_sources})
	endif()
...
function(unigraph_unit unit_name)
    cmake_parse_arguments(
        ...
        ""
        "TYPE"
        "SOURCES;HEADERS;DEPEND;NAME;INCLUDE_DIRS;TEST_SOURCES;NOLINK_DEPEND;PLATFORM_SOURCES"
        ${ARGN}
    )
    ...
    set(linux_sources)
    set(darwin_sources)
    if (PARSED_ARGS_PLATFORM_SOURCES)
      foreach (source IN LISTS PARSED_ARGS_PLATFORM_SOURCES)
	if (source STREQUAL ":windows")
	  set(current_platform "windows_sources")
	elseif (source STREQUAL ":linux")
	  set(current_platform "linux_sources")
	elseif (source STREQUAL ":darwin")
	  set(current_platform "darwin_sources")
	else ()
	  if (NOT current_platform STREQUAL "")
	    list(APPEND ${current_platform} "${_UNIGRAPH_CURRENT_UNIT_DIRECTORY}/${source}")
	  else ()
	    _unigraph_message(FATAL_ERROR "Invalid PLATFORM_SOURCES format: '${source}' must be preceded by a platform specifier (:windows, :linux, :darwin)")
	  endif ()
	endif ()
      endforeach ()
    endif ()
    ...
    if (CMAKE_SYSTEM_NAME STREQUAL "Windows" AND windows_sources)
      set(target_type "StaticLibrary")
    elseif (CMAKE_SYSTEM_NAME STREQUAL "Linux" AND linux_sources)
      set(target_type "StaticLibrary")
    elseif (CMAKE_SYSTEM_NAME STREQUAL "Darwin" AND darwin_sources)
      set(target_type "StaticLibrary")
    endif ()
    ...
    _unigraph_set_value_to_dict(LIST unit "windows_sources" "${windows_sources}")
    _unigraph_set_value_to_dict(LIST unit "linux_sources" "${linux_sources}")
    _unigraph_set_value_to_dict(LIST unit "darwin_sources" "${darwin_sources}")
